...
class Model extends BaseModel
{
    ...
    public function __construct(?ConnectionInterface $db = null, ?ValidationInterface $validation = null)
    {
        ...
        if ($dbtype == 'oracle') {
            ...
            $this->allowedFields = array_map('strtoupper', $this->allowedFields);
            $this->primaryKey = strtoupper($this->primaryKey);
        } else {
            $this->table = strtolower($this->table);
            $this->allowedFields = array_map('strtolower', $this->allowedFields);
        }
    }
    ...
    protected function doFindAll(?int $limit = null, int $offset = 0)
    {
        ...
        if ($dbtype == 'oracle') {
            foreach ($results as $i => $row) {
                $results[$i] = array_change_key_case($row, CASE_LOWER);
            }
        }
    ...
    {
        $dbtype = getenv('app.dbtype');
        if ($dbtype == 'oracle' && $name == 'select') {
            $params = array_map('strtoupper', $params);
        }
        if ($name == 'where' || $name == 'orWhere' || $name == 'having' || $name == 'orHaving' || $name == 'whereIn' || $name == 'orWhereIn' || $name == 'havingIn' || $name == 'orHavingIn' || $name == 'whereNotIn' || $name == 'orWhereNotIn' || $name == 'havingNotIn' || $name == 'orHavingNotIn' || $name == 'like' || $name == 'notLike' || $name == 'orLike' || $name == 'orNotLike' || $name == 'havingLike' || $name == 'havingNotLike' || $name == 'orHavingLike' || $name == 'orHavingNotLike') {
            if ($dbtype == 'oracle')
                if (is_string($params[0])) {
                    ...
                    foreach ($params as $key => $value) {
                        foreach ($value as $k => $v) {
                            $params[$key][strtoupper($k)] = $v;
                            unset($params[$key][$k]);
                        }
                    }
                }
        }
        if ($name == 'join' && $dbtype == 'oracle') {
            // make all values uppercase
            $params = array_map('strtoupper', $params);
            // if params[1] contains quotes, make inside of quotes lowercase
            $params[1] = preg_replace_callback(
                '/\'(.*?)\'/',
